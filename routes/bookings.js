...
router.post('/', auth, async (req, res) => {
  try {
    // BLOCK if ANY pending fine: cancellationFee > 0 && !cancellationFeeConfirmed
    ...
    if (pendingFines > 0) {
      return res.status(403).json({ 
        msg: 'Cannot create new bookings. You have pending cancellation fee confirmation from provider(s).' 
      });
    }
